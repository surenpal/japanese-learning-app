...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white dark:bg-gray-950">
      {/* Header */}
      <header className="border-b border-gray-100 dark:border-gray-800 px-6 py-4 flex items-center justify-between max-w-7xl mx-auto">
        <div className="flex items-center gap-2">
          <span className="text-3xl">🇯🇵</span>
          <div>
            <p className="font-bold text-lg text-gray-900 dark:text-gray-100 leading-tight">Nihongo Master</p>
            <p className="text-xs text-gray-400 dark:text-gray-500">Learn Japanese for JLPT · JFT · Skills Test</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <ThemeToggle />
      ...
      <section className="max-w-6xl mx-auto px-6 pb-24">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(({ icon: Icon, title, desc, color }) => (
            <div
              key={title}
              className="rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-6 shadow-sm hover:shadow-md transition-shadow"
            >
      ...
      {/* Reference sites */}
      <section className="bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 py-12 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <p className="text-sm text-gray-400 dark:text-gray-500 mb-3">Inspired by the best Japanese learning resources</p>
          <div className="flex flex-wrap justify-center gap-4 text-sm text-gray-500 dark:text-gray-400">
            {["JLPT Pro", "Tofugu", "TryJLPT", "Irodori", "JapaneseTest.org", "Anki"].map((s) => (
              <span key={s} className="px-3 py-1 rounded-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                {s}
              </span>
            ))}
          </div>
